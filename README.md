...
This research was conducted based on the results of Hongwei Wang (the first author) ’s research intern in DiDi Chuxing. The data of the research were extracted from the commercial cloud database of DiDi, and thus were temporarily classified due to concerns of privacy and information security. We already have made efforts to publish part of the data, but the application was still in process in DiDi. Alternatively, researchers can apply for a full traffic dataset by themselves from GAIA Open Dataset Initiative of DiDi, which is an open dataset project to provide academic community with real-life application use cases, anonymized data and computing resources. More details of GAIA Open Dataset Initiative are available in
```bash
...
The two config files containing specific parameters to generate meteorological data in Shenzhen is listed in the WRF documents in `DCRNN_DS_Github/WRF`: `namelist_shenzhen.input`, `namelist_shenzhen.wps`
...
(5) 70% of the data is used for training, 20% is used for testing, and the rest 10% for validation.  
...
```bash
DCRNN_DS_Github/data_preprocessing/wrf_extraction_shenzhen.py
```
...
Codes can be checked here: 
```bash
DCRNN_DS_Github/data_preprocessing/merge_data_and_build_input.py
```
...
2. Loss functions. The loss function cannot be directly changed in `dcrnn_sz.yaml`. You need to change the codes in `dcrnn_supervisor.py`, `Line 79, 233-234, 285-294` to other loss functions. Remember, import the metrics first, like `from lib.metrics import masked_mae_loss, masked_rmse_loss, masked_mse_loss`

3. Hyper-parameters. The major hyper-meter of DCRNN are iffusion stes, nodes and layers, which can be tuned in `data/model/dcrnn_sz.yaml`. The name of the file of a well-trained model means `diffusion_step-timestep-nodes(layers)-learning_rate-batchsize`, e.g. dcrnn_DR_1_h_24_256-256_lr_0.01_bs_4. 
...
We thank Li et al. to publish their codes of original DCRNN to facilitate this research field, which is cited from:
